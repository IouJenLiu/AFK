...
from .utils import on_danger_zone
        ...
        for i in range(env.grid.width):
            for j in range(env.grid.height):
                ...
                if self.type is not None and cell.type != self.type:
                    if cell.type == 'box' and cell.contains:
                        if cell.contains.type != self.type:
                            continue
                    else:
                        continue
                ...
                if self.color is not None and cell.color != self.color:
                    if cell.type == 'box' and cell.contains:
                        if cell.contains.color != self.color:
                            continue
                    else:
                        continue
...
class OpenInstr(ActionInstr):
    ...
    def surface(self, env):
        return 'find the key and open ' + self.desc.surface(env)
...
class OpenInstrGTAns(OpenInstr):
    def __init__(self, obj_desc, ans, strict=False):
        super().__init__(obj_desc, strict)
        self.ans = ans
...
class FindInstr(ActionInstr):
    ...
    def surface(self, env):
        return 'find ' + self.desc.surface(env)
    ...
    def verify_action(self, action):
        ...
        if front_cell and front_cell.color == self.desc.color and front_cell.type == self.desc.type:
            return 'success'
        else:
            return 'continue'


class FindOrFailInstr(ActionInstr):
    """
    Go next to (and look towards) an object matching a given description
    eg: go to the door
    """

    def __init__(self, obj_desc, surface=None, danger=False, door=False):
        super().__init__()
        self.desc = obj_desc
    ...
    def verify_action(self, action):
        if self.danger and on_danger_zone(self.env):
            return 'failure'
        linear_idx = self.env.front_pos[1] * self.env.grid.width + self.env.front_pos[0]
        front_cell = self.env.grid.grid[linear_idx]
        if front_cell and front_cell.color == self.desc.color and front_cell.type == self.desc.type:
            return 'success'
        elif front_cell and front_cell.type == self.desc.type and (not front_cell.color == self.desc.color):
            return 'failure'
        else:
            if self.open_door and action == self.env.actions.toggle:
                # Get the contents of the cell in front of the agent
                front_cell = self.env.grid.get(*self.env.front_pos)
                for door in self.env.target_doors:
                    if front_cell and front_cell is door and door.is_open:
...
class GoToGTAns(ActionInstr):
    """
    Go next to (and look towards) an object matching a given description
    eg: go to the door
    """

    def __init__(self, obj_desc, ans):
        super().__init__()
        self.desc = obj_desc
        self.ans = ans
...
class FavoriteInstr(GoToInstr):
    """
    Go next to (and look towards) an object matching a given description
    eg: go to the door
    """

    def __init__(self, obj_desc, name, surface=None, danger=False, open_door=False):
        super().__init__(obj_desc)
        self.desc = obj_desc
        self.name = name
        self.surface_ = surface
        self.danger = danger
    ...
    def surface(self, env):
        if self.surface_:
            return self.surface_
        return "go to {} toy".format(self.name)
...
class GoToGoalInstr(ActionInstr):
    ...
    def __init__(self, door=False):
        super().__init__()
        self.door = door

    def surface(self, env):
        if self.door:
            return 'Avoid danger zone, find the key and open the door, go to the green square'
        else:
            return 'Avoid danger zone, go to the green square'

    def reset_verifier(self, env):
        super().reset_verifier(env)

    def verify_action(self, action):
        linear_idx = self.env.agent_pos[1] * self.env.grid.width + self.env.agent_pos[0]
        cur_cell = self.env.grid.grid[linear_idx]
        if cur_cell and cur_cell.type == 'lava' and cur_cell.color != self.env.lava_colors[self.env.target_idx]:
            return 'failure'
        if tuple(self.env.unwrapped.agent_pos) == self.env.unwrapped.goal_pos:
            return 'success'
        if self.door and action == self.env.actions.toggle:
            # Get the contents of the cell in front of the agent
            front_cell = self.env.grid.get(*self.env.front_pos)
            for door in self.env.target_doors:
                if front_cell and front_cell is door and door.is_open:
