...
class RoomGridLevel(RoomGrid):
    ...
    def reset(self, **kwargs):
        ...
        self.partial_succ = False
        return obs

    def step(self, action):
        if self.front_pos[0] < 0 or self.front_pos[0] >= self.width:
            print('')
        ...
        if status == 'success':
            ...
            reward = -0.5 if self.failure_neg else 0
        elif status == 'continue':
            done = False
        elif status == 'partial_success':
            done = False
            if not self.partial_succ:
                reward = self._reward()
            self.partial_succ = True
