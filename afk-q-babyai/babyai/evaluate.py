...
from babyai.utils.wrapper import TransformReward, DirectionWrapper, AnsWrapper, XYLocationWrapper, InstrWrapper, KGWrapper
...
# Returns the performance of the agent on the environment for a particular number of episodes.
def batch_evaluate(agent, env_name, seed, episodes, return_obss_actions=False, pixel=False, query=False, restricted_query=False, args=None):
    ...
    for i in range((episodes + num_envs - 1) // num_envs):
        ...
        while (num_frames == 0).any():
            ...
            many_obs, reward, done, info = env.step(action)
            agent.analyze_feedback(reward, done)
            done = np.array(done)
            ...
            for i in range(num_envs):
                if just_done[i]:
                    logs['success'].append(info[i]['success'])
    ...
    #print('Good questions per episode: {}'.format(good_question / episodes))
    #print('questions per episode: {}'.format(question / episodes))
    return logs
