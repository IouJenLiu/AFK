...
from babyai.utils.distribution import MultiCategorical
...
class ModelAgent(Agent):
    ...
    def __init__(self, model_or_name, obss_preprocessor, argmax, query=False, postprocess_action=None):
    ...
    def act_batch(self, many_obs):
        ...
        with torch.no_grad():
            model_results = self.model(preprocessed_obs, self.memory, vocab=self.obss_preprocessor.vocab)
...
def load_agent(env, model_name, demos_name=None, demos_origin=None, argmax=True, env_name=None, query=False, postprocess_action=None):
    # env_name needs to be specified for demo agents
    if model_name == 'BOT':
        return BotAgent(env)
    elif model_name is not None:
        obss_preprocessor = utils.ObssPreprocessor(model_name, env.observation_space, query=query)
        return ModelAgent(model_name, obss_preprocessor, argmax, query=query, postprocess_action=utils.PointerActionPostprocessor(obss_preprocessor.vocab))
