...
class PPOAlgo(BaseAlgo):
    ...
    def __init__(self, envs, acmodel, num_frames_per_proc=None, discount=0.99, lr=7e-4, beta1=0.9, beta2=0.999,
                 ...
                 reshape_reward=None, aux_info=None, postprocess_action=None, update_query_only=False, tvt=False,
                 bin_entropy_coef=0):
        num_frames_per_proc = num_frames_per_proc or 128

        super().__init__(envs, acmodel, num_frames_per_proc, discount, lr, gae_lambda, entropy_coef,
                         value_loss_coef, max_grad_norm, recurrence, preprocess_obss, reshape_reward,
                         aux_info, postprocess_action, tvt=tvt)

        self.clip_eps = clip_eps
        self.epochs = epochs
        self.batch_size = batch_size
        self.bin_entropy_coef = bin_entropy_coef

        assert self.batch_size % self.recurrence == 0
        if update_query_only:
            self.optimizer = torch.optim.Adam(list(self.acmodel.actor_query.parameters()) + list(self.acmodel.critic.parameters()), lr, (beta1, beta2), eps=adam_eps)
        else:
    ...
    def update_parameters(self):
        ...
        for _ in range(self.epochs):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                for i in range(self.recurrence):
                    ...
                    model_results = self.acmodel(sb.obs, memory * sb.mask, vocab=self.preprocess_obss.vocab)
                    ...
                    if model_results['bin_dist'] is not None:
                        bin_entropy = model_results['bin_dist'].entropy().mean()
                        loss -= self.bin_entropy_coef * bin_entropy
        ...
        logs["action_prob"] = torch.mean(dist.probs.detach(), dim=0).cpu().numpy()
        return logs
