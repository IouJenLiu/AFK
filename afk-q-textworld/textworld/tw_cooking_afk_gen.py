...
SKILLS = ["recipe", "take", "cook", "cut", "open", "drop"]
...
FOODS_COMPACT = {
    "egg": {
        ...
        "locations": ["kitchen.fridge", "kitchen.showcase"],
    },
    "milk": {
        "indefinite": "some",
    ...
    "meat": {
        "indefinite": "a",
        "names": ["pork meat", "chicken meat", "beef meat", "veggie meat"],
        "properties": ["inedible", "cookable", "needs_cooking"],
        "locations": ["kitchen.fridge", "supermarket.showcase"],
    },
    "tuna": {
        "indefinite": "a",
        "names": ["red tuna", "white tuna", "pink tuna"],
    ...
    "tomato": {
        "indefinite": "a",
        "names": ["red tomato", "yellow tomato", "green tomato"],
        "properties": ["edible", "cookable", "raw", "cuttable", "uncut"],
        "locations": ["kitchen.counter", "garden"],
    },
    "pepper": {
        "names": ["hot pepper", "green pepper", "bell pepper", "yellow pepper", "red pepper"],
        "indefinite": "a",
        "properties": ["edible", "cookable", "raw", "cuttable", "uncut"],
        "locations": ["kitchen.counter", "garden"],
    },
...
FURNITURES = {
    ...
    "fridge": {
        "type": "c",
        "names": ["fridge", "refrigerator", "freezer"],
        "adjs": ["stainless", "aluminum", "old", "new"],
        "locations": ["kitchen"],
        "properties": ["closed"],
        "desc": [None],
    },
    ...
    "cart": {
        "type": "c",
        "names": ["cart"],
        "adjs": ["serving", "tea", "utility", "new", "old"],
        "locations": ["kitchen"],
        "properties": ["closed"],
        "desc": [None],
    },
    "cabinet": {
        "type": "c",
        "names": ["cabinet", "cupboard"],
        "adjs": ["wooden", "marble", "stainless", "upper", "lower"],
        "locations": ["kitchen"],
        "properties": ["closed"],
        "desc": [None],
    },
    "drawer": {
        "type": "c",
        "names": ["drawer"],
        "adjs": ["wooden", "marble", "stainless", "bottom", "middle", "top"],
        "locations": ["kitchen"],
        "properties": ["closed"],
        "desc": [None],
    }
...
ENTITIES = dict(**FURNITURES, **OBJECTS)
...
def pick_location(M, locations, rng):
    locations = list(locations or [e.name for e in M._holders if e])
...
def place_food(M, name, rng, place_it=True):
    holder = pick_location(M, None, rng)
...
def place_random_foods(M, nb_foods, rng, allowed_foods=FOODS):
    ...
    foods = sorted([name for name in allowed_foods if name not in seen])
    rng.shuffle(foods)
    entities = []
    for food in foods:
...
def place_entity(M, name, rng) -> WorldEntity:
    ...
    entity.infos.indefinite = ENTITIES[name].get("indefinite", "a")
...
def place_random_furnitures(M, nb_furnitures, rng):
    ...
    furnitures = [furniture for furniture in sorted(FURNITURES) if not M.find_by_name(furniture)]
...
def make(settings: Mapping[str, str], options: Optional[GameOptions] = None) -> textworld.Game:
    ...
    # Load knowledge base specific to this challenge if not already loaded.
    options.kb = options._kb or KnowledgeBase.load(logic_path=KB_LOGIC_PATH, grammar_path=KB_GRAMMAR_PATH)
    ...
    # Create the only room: kitchen.
    kitchen = M.new_room("kitchen")
    rooms = [kitchen]

    # Keep track of possible holders for ingredients.
    M._holders = [kitchen]
    ...
    M._holders += place_entities(M, ["stove", "oven", "counter", "fridge", "griller"], rng_objects)

    # Place some random furnitures.
    nb_furnitures = settings.get("nb_furniture_distractors", 0)
    M._holders += place_random_furnitures(M, nb_furnitures, rng_objects)
    ...
    if rng_objects.rand() > 0.5 or settings.get("cut"):
        ...
        if settings.get("knife_in_inventory"):
            M.move(knife, M.inventory)
    ...
    # Remove every food preparation with grilled, if there is no griller.
    if M.find_by_name("griller") is None:
    ...
    # Add distractors foods. The amount is drawn from N(nb_food_distractors, 3).
    #nb_distractors = abs(int(rng_objects.randn(1) * 3 + nb_ingredients + settings.get("nb_food_distractors", 0)))
    nb_distractors = settings.get("nb_food_distractors", 0)
    ...
    cutting_ingredient_facts = []
    cooking_ingredient_facts = []
    for i, ingredient in enumerate(ingredients):
        ...
        if ingredient[0] not in M.inventory:
            ...
            quests.append(Quest(win_events=[holding_ingredient], reward=0))
        ...
        if ingredient[1] != TYPES_OF_COOKING[0] and not ingredient[0].has_property(ingredient[1]):
            ...
            cooking_ingredient_facts += [M.new_fact(ingredient[1], ingredient[0])]

        fail_events = [Event(conditions={M.new_fact(t, ingredient[0])})
                       for t in set(TYPES_OF_COOKING[1:]) - {ingredient[1]}]  # Wrong cooking.

        quests.append(Quest(win_events=win_events, fail_events=[ingredient_consumed] + fail_events, reward=0))
        ...
        if ingredient[2] != TYPES_OF_CUTTING[0] and not ingredient[0].has_property(ingredient[2]):
            ...
            cutting_ingredient_facts += [M.new_fact(ingredient[2], ingredient[0])]
            assert ingredient[2] != "uncut"
    ...
    holding_ingredient_facts = [M.new_fact("in", ingredient[0], M.inventory) for ingredient in ingredients]
    holding_all_ingredients_processed_correctly_event = Event(
        conditions=holding_ingredient_facts + cooking_ingredient_facts + cutting_ingredient_facts
    )
    quests.append(Quest(win_events=[holding_all_ingredients_processed_correctly_event], reward=1))
    ...
    if settings.get("cook"):
        for food, type_of_cooking, _ in ingredients:
            if type_of_cooking == "fried":
                ...
                griller = M.find_by_name("griller")
                walkthrough.append("cook {} with {}".format(food.name, griller.name))
    ...
    if settings.get("cut"):
        ...
        if knife:
            ...
            if knife.parent.name == "kitchen":
                walkthrough.append("take {}".format(knife.name))
            elif knife.parent == M.inventory:
                pass
            else:
                if knife.parent.has_property("closed"):
                    walkthrough.append("open {}".format(knife.parent.name))

                walkthrough.append("take {} from {}".format(knife.name, knife.parent.name))
    ...
    # Manually change the name and description for some of the entities.
    kitchen.infos.adj = "messy"
    kitchen.infos.noun = "kitchen"
    kitchen.infos.name = "messy kitchen"
    ...
    list_of_ingredients = ", ".join(food.name for food, _, _ in ingredients)
    idx = list_of_ingredients.rfind(", ")
    if idx >= 0:
        list_of_ingredients = list_of_ingredients[:idx] + ", and " + list_of_ingredients[idx + 2:]

    objective = ("You find yourself at friend's house and you are both hungry! Collect some {ingredients} to prepare a delicious meal."
                 " Ask {name} to know where to find each ingredient and for the recipe directions.")
    game.objective = objective.format(ingredients=list_of_ingredients, name=settings["oracle_name"])

    game.metadata = metadata
    skills_uuid = "+".join("{}{}".format(k, "" if settings[k] is True else settings[k])
                           for k in SKILLS if k in settings and settings[k])
    uuid = "tw-cooking-afk-{specs}-{seeds}"
